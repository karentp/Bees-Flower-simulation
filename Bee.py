class Bee():
    ...
    orientations = ["N", "S", "E", "O", "NE",
                    "NO", "SE", "SO"]  # 8 direcciones
    ...
    def make_cromosoma(self):
        ...
        cromosoma = ""
        cromosoma += self.convert_to_binary(self.second_search,
                                            "secondary_search")
        cromosoma += self.convert_to_binary(
            self.paths.index(self.path), "path")
        cromosoma += self.convert_to_binary(
            self.radios.index(self.radio), "radio")
        cromosoma += self.convert_to_binary(
            self.angles.index(self.angle), "angle")
        cromosoma += self.convert_to_binary(
            self.orientations.index(self.orientation), "orientation")
        cromosoma += self.convert_to_binary(
            self.colors.index(self.color), "color")
    ...
    def adaptability_function(self):
        self.nota_seleccion = (
            len(self.flores_visitadas) / self.distancia_recorrida)*100

        return self.nota_seleccion
